# This program reads a string and checks n times whether substrings of length x starting at positions p1 and p2 are the same
...
k = int(input())
for k in range(k):
    length, first, second = map(int, input().split())
    first += 1
    second += 1
    if are_same(first, second, length):
        print('yes')
    else:
        print('no')
